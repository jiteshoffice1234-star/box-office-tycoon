...
import { makeSequel, addDirectorCommentary, signMerchandiseDeal } from '../game/engine'
                ...
                <tr key={m.id}>
                  <td>
                    <div className="cell-title">
                      {m.isDisaster && <span className="dist-badge" style={{background:'#c0392b',color:'#fff'}}>DISASTER</span>}
                      {m.phase === 'evergreen' && <span className="hired-badge" style={{background:'#27ae60',color:'#fff'}}>EVERGREEN</span>}
                      {m.contentType === 'series' && <span className="hired-badge" style={{background:'#2980b9',color:'#fff'}}>📺 SERIES</span>}
                      {m.contentType === 'show' && <span className="hired-badge" style={{background:'#8e44ad',color:'#fff'}}>📡 SHOW</span>}
                  ...
                  <td>
                    ...
                    {m.owner === 'player' && m.quality >= 60 && !m.commentaryActive && m.phase !== 'preProduction' && (
                      <Btn small onClick={() => apply((s) => addDirectorCommentary(s, m.id))}>
                        💬 Commentary
                      </Btn>
                    )}
                    {m.owner === 'player' && m.quality >= 70 && !m.hasMerchandise && m.totalGross > m.cost * 2 && m.finished && (
                      <Btn small onClick={() => apply((s) => signMerchandiseDeal(s, m.id))}>
                        🧸 Merch
                      </Btn>
                    )}
                  </td>
